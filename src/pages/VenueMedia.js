...
function Home() {
  ...
  const deleteImage = (index) => {
    ...
    axios.post(url + 'venue/image/delete/' + id, { images: images, deletedImage: dlImages }).then(resp => {
      axios.post(url + 'venue/' + id, {}).then(resp => {
        let response = structuredClone(resp?.data?.data)
        console.log(response.images)
        setState(response)
  ...
  const onFileUpload = () => {
    ...
    axios.post(url + 'venue/image/upload/' + id, formData).then(resp => {
      axios.post(url + 'venue/' + id, {}).then(resp => {
        let response = structuredClone(resp?.data?.data)
        console.log(response.images)
        setState(response)
      ...
      <main>
        ...
        <section id='page-content'>
          ...
          <div className='main-area'>
            <div id='page-title'>
              <h1>Venue Media <small><small><small>({state.name})</small></small></small></h1>
            </div>
