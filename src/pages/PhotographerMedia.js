...
function Home() {
  ...
  const deleteImage = (index) => {
    ...
    axios.post(url + 'photographer/image/delete/' + id, { images: images, deletedImage: dlImages }).then(resp => {
      axios.post(url + 'photographer/' + id, {}).then(resp => {
        let response = structuredClone(resp?.data?.data)
        console.log(response.images)
        setState(response)
  ...
  const onFileUpload = () => {
    ...
    axios.post(url + 'photographer/image/upload/' + id, formData).then(resp => {
      axios.post(url + 'photographer/' + id, {}).then(resp => {
        let response = structuredClone(resp?.data?.data)
        console.log(response.images)
        setState(response)
      ...
      <main>
        ...
        <section id='page-content'>
          ...
          <div className='main-area'>
            <div id='page-title'>
              <h1>Photographer Media <small><small><small>({state.name})</small></small></small></h1>
            </div>
