...
function Home() {
  ...
  let [state, setState] = useState({ photographersShow: [], page: 1});

  useEffect(() => {
    axios.post(url + 'photographers', {}).then(resp => {
      setState({ ...state, photographers: resp?.data?.data, photographersShow :   resp?.data?.data})
    })
  }, [])
  ...
  const deleteItemConfirm = (i) => {
    ...
    axios.post(url + 'photographer/delete/' + state.deleteRef._id, {}, {
      ...
      if (resp.data.status != 'error') {
        axios.post(url + 'photographers', {}).then(resp => {
          setState({ ...state,  photographers: resp?.data?.data, photographersShow : resp?.data?.data,  deleteRef: null, showDeleteModal: false, })
  ...
  useEffect(()=>{
    if(state.searchKey){
      let vanues = JSON.parse(JSON.stringify(state.photographers));
      ...
      console.log(vanues)
      setState({...state, photographersShow: vanues})
    } else {
      setState({...state, photographersShow: state.photographers})
    }

  },[state.searchKey])

  useEffect(() => {
    axios.post(url + 'photographers?page='+state.page, {}).then(resp => {
      setState({ ...state, photographers: resp?.data?.data, photographersShow: resp?.data?.data, searchKey: '' })
    })
          ...
          <div className='main-area'>
            ...
            <div className='working-area'>
              ...
              <div className='row'>
                ...
                {state.photographersShow && state.photographersShow.length ? <div class="table-responsive">
                  ...
                  <table class="table table-hover table-bordered table table-striped">
                    ...
                    <tbody>
                      {
                        state.photographersShow.map((item, index) => {
                          return (
                            <tr>
                              <td>{item.name}</td>
                              <td>{item.type}</td>
              ...
              <nav aria-label="Page navigation example">
                <ul class="pagination">
                  ...
                  { state.photographersShow?.length ? <li class="page-item" ><button onClick={() => { setState({ ...state, page: state.page + 1 }) }} class="page-link" href="#">Next</button></li> : null }
                </ul>
              </nav>
              {/*  <!-- Working area end--> */}
            </div>
