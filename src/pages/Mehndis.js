...
function Home() {
  ...
  let [state, setState] = useState({ mehndisShow : [], page: 1});

  useEffect(() => {
    axios.post(url + 'mehndi', {}).then(resp => {
      setState({ ...state, mehndis: resp?.data?.data ,  mehndisShow: resp?.data?.data,})
    })
  }, [])
  ...
  const deleteItemConfirm = (i) => {
    ...
    axios.post(url + 'mehndi/delete/' + state.deleteRef._id, {}, {
      ...
      if (resp.data.status != 'error') {
        axios.post(url + 'mehndi', {}).then(resp => {
          setState({ ...state, mehndis: resp?.data?.data, mehndisShow: resp?.data?.data,   deleteRef: null, showDeleteModal: false, })
  ...
  useEffect(()=>{
    if(state.searchKey){
      let vanues = JSON.parse(JSON.stringify(state.mehndis));
      ...
      console.log(vanues)
      setState({...state, mehndisShow: vanues})
    } else {
      setState({...state, mehndisShow: state.mehndis})
    }

  },[state.searchKey])

  useEffect(() => {
    axios.post(url + 'mehndi?page='+state.page, {}).then(resp => {
      setState({ ...state, mehndis: resp?.data?.data, mehndisShow: resp?.data?.data, searchKey: '' })
    })
          ...
          <div className='main-area'>
            ...
            <div className='working-area'>
              ...
              <div className='row'>
                ...
                {state.mehndisShow && state.mehndisShow.length ? <div class="table-responsive">
                  ...
                  <table class="table table-hover table-bordered table table-striped">
                    ...
                    <tbody>
                      {
                        state.mehndisShow.map((item, index) => {
                          return (
                            <tr>
                              <td>{item.name}</td>
                              <td>{item.type}</td>
              ...
              <nav aria-label="Page navigation example">
                <ul class="pagination">
                  ...
                  { state.mehndisShow?.length ? <li class="page-item" ><button onClick={() => { setState({ ...state, page: state.page + 1 }) }} class="page-link" href="#">Next</button></li> : null }
                </ul>
              </nav>
              {/*  <!-- Working area end--> */}
            </div>
